...
const playMusic = (track, pause = false) => {
    currentSong.src = "songs/" + track; // Adjust the path as necessary
    if (!pause) {
        currentSong.play();
        play.src = "pause.svg";
    }
...
async function main() {
    ...
    if (songs.length < 2) {
        console.error("Not enough songs to play. Please add at least two songs.");
        return; // Exit if there are less than 2 songs
    }
    playMusic(songs[0], true); // Play the first song

    // Show all in the playlist
